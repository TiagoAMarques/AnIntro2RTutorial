...
To start using this tutorial right away download the contents of the [repository](https://github.com/TiagoAMarques/AnIntro2RTutorial). At a bare minimum, you should first look at the slides at "AQuikIntro2RandRStudioInQuarto.html" available at Rpubs [here](https://rpubs.com/talomarques/AnIntroToRandRStudio). If you are new to RStudio this presentation can be helpful in getting you started. If you are a student of one of my courses at FCUL, this presentation will be delivered to you in class.

Then, download (at a bare minimum) these three files

- `TAMsIntro2RviaRStudioTutorial.pdf`
- `dados1.csv`
- `mydatafile.txt`

Then:

1.  Open the PDF or HTML tutorial.
2.  Follow the tutorial in order, running the code examples in RStudio.
3.  Modify the code as you go, so you can experiment and see how R behaves.

The file `TAMsIntro2RviaRStudioTutorial.pdf` was generated by compiling\
`TAMsIntro2RviaRStudioTutorial.qmd`, which is a dynamic report written in [Quarto](https://rmarkdown.rstudio.com/). There are corresponding and `TAMsIntro2RviaRStudioTutorial.html` and `TAMsIntro2RviaRStudioTutorial.docx` files if you prefer to work from these.

# Who is this for

I use this material for both my `Ecologia Numérica` and my `Modelação Ecológica` at [FCUL](https://ciencias.ulisboa.pt/en) courses. If you are a student in one of those courses, please bear in mind that the section `Final tasks` is only intended to be completed for those in `Modelação Ecológica`.
...
!["Artwork from "Hello, Quarto" keynote by Julia Lowndes and Mine Çetinkaya-Rundel, presented at RStudio Conference 2022. Illustrated by Allison Horst."](images/clipboard-3012093320.png)
...
If you want to learn R interactively from the command line, you might want to try package [Swirl](https://swirlstats.com/). Your own R tutor at your fingertips. Try it!
...
- [Modern Statistics with R](https://modernstatisticswithr.com/) by Mans Thulin, Modern Statistics with R: From wrangling and exploring data to inference and predictive modelling (2024-07-01 - Second edition - Version 2.0.0)

- [Material from Gavin Simpson's course on GAMs](https://github.com/gavinsimpson/physalia-gam-course) A course on Generalized Additive Models; a data-driven approach to estimating regression models This is part of a suite of Physalia-Courses hosted at <https://www.physalia-courses.org/>
