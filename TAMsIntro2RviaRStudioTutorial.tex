...
You might be reading this document as a pdf, a word or an html. These
files are created by compiling (i.e.~knitting) a quarto dynamic report
(see many more details about dynamic reports below). The .qmd
``TAMsIntro2RviaRStudioTutorial.qmd'', which after compilation, creates
these 3 different formats, is available in the github repository located
at:
...
This .qmd was last compiled on 2026-09-20 13:42:54.686843. The latest
version of this tutorial is always available at the
\href{https://github.com/TiagoAMarques/AnIntro2RTutorial}{github
repository}.
...
short, make them as short and easily readable as possible.

Also, you should be aware that some directories names can cause issues.
If you are a user with those characters in the name, and you are working
via a user with your name, then this applies to you.

Finally, be careful about some directories not having necessarily the
name (for the system) that Windows shows you to have (examples for PT
users: ``Utilizador'' vs ``User'' and ``desktop'' vs ``Ambiente de
trabalho'').
...
packages is recommended. Both are free and open source. You can get R
\href{https://www.r-project.org/}{here} and Rstudio
\href{https://www.rstudio.com/}{here}.

It is fundamental that the difference between these two pieces of
software is clear in your mind from the start:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  R is the workhorse that does all the work in the background,
\item
  RStudio is just a user interface that allows you to access all the
  power of R in a convenient way.
\end{enumerate}

Additional R packages will be later installed, but you will learn about
that in due time :)

\section{Introduction}\label{introduction}

This tutorial was created as a gentle introduction to the R environment
via the RStudio interface to R.

While it could be a general introduction to R, the primary objective of
this document is to serve as a ``hands-on-tutorial'' for courses
delivered by me (TAM). I use it for both Ecologia Numérica and Modelação
Ecológica, courses for biology undergraduate and MSc students,
respectively, at the Department of Biology (DB) at Faculdade de Ciências
da Universidade de Lisboa (FCUL).

This material does not assume any previous knowledge about R, but some
basic logic and programming notions would be desirable.

To facilitate the interaction with R we leverage on
\href{https://www.rstudio.com/}{RStudio}, a graphical user interface
(GUI), a piece of software which allows users to have at a click's
distance many useful features of R. In the following sections of the
tutorial you will be guided through a first session of R via RStudio,
and then you will start working on a dynamic report.

The tutorial is intended to follow a presentation about R and RStudio,
their interaction and capabilities
(``AQuikIntro2RandRStudioInQuarto.html''). This document is also in the
git repository, but the presentation itself is also hosted at RPubs:
\href{https://rpubs.com/talomarques/1342657}{A Quick Introduction to R
and RStudio} where you can view it immediately.

If you are using this tutorial in class with me/a teacher, after you are
exposed to the slides, you might want to jump straight to section
Section~\ref{sec-starthere2}. Do come back after class to all the
material in between here and there, as it might contain useful context
and additional relevant material. Remember, here you learn as much as
you want to, and those that want the most will certainly do better.
...
'@'allison\_horst, and I recommend you visit Allison's
\href{https://github.com/allisonhorst/stats-illustrations}{github
repository} filled up with amazing stats and maths illustrations,
including so many amazing resources to make R look less frightening. To
be honest, this section is actually also an homage to Allison's
outstanding work.

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{extfiles/monsteRs.jpg}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{extfiles/derivative_1.jpg}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{extfiles/monster_support.jpg}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{extfiles/r_first_then.png}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{extfiles/usethis.png}}
...
This document is written in Quarto, a tool that allows you to build
...
results. If you want a gentle introduction to Quarto using a hands on
tutorial based on a versatile template that will do many of the things
you will need to get started, look for no more, there is one here:

\url{https://github.com/TiagoAMarques/template_4_quarto_report}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{extfiles/code_hero.jpg}}
...
\item
  ...
  functions
\item
  \href{https://rstudio.github.io/cheatsheets/}{Cheat sheets} - an
  incredible useful set of resources from the RStudio team, where self
  contained subject specific sets of functions are provided for
  different common tasks. Because of their importance for our work, we
  ...
  \item
    ...
    R}: the basics in base R
  \item
    \href{https://rstudio.github.io/cheatsheets/quarto.pdf}{quarto}:
    making dynamic reports in quarto
  \item
    \href{https://rstudio.github.io/cheatsheets/html/lubridate.html}{lubridate}:
    manipulating and formating dates and times
  \item
    \href{https://rstudio.github.io/cheatsheets/html/data-visualization.html}{ggplot}:
    plotting data
  \item
    \href{https://rstudio.github.io/cheatsheets/html/data-transformation.html}{dplyr}:
    manipulating data
  \item
    \href{https://rstudio.github.io/cheatsheets/sf.pdf}{sf}:
...
cheat sheets} also exist.

At the \href{https://github.com/TiagoAMarques/AnIntro2RTutorial}{landing
page} of the github repository hosting this tutorial there is a longer
list of less introductory/general resources on R that might just have
what you were looking for. Disclaimer: said list corresponds to a random
non-exhaustive list of resources I have read and were useful to me at
some point. I make no claims they might be useful to you :)

\section{Base R vs the tidyverse}\label{base-r-vs-the-tidyverse}

There are essentially two paradigms to coding in R. They are often as
two religions, in more than one respect, so try not to get caught in the
middle of a religious battle between them. On the developers own words,
``The tidyverse is an \textbf{opinionated} collection of R packages
designed for data science. All packages share an underlying design
philosophy, grammar, and data structures''. I am R user pragmatic, I
take whatever works for me in any given scenario. I was educated in base
R, but the tidyverse can be quite useful.

The \texttt{tidyverse} way can be implemented via a suite of R packages,
and you can read all about it in this
\href{https://r4ds.hadley.nz/}{book}.

\pandocbounded{\includegraphics[keepaspectratio]{images/clipboard-2143144059.png}}

Using one or the other will lead to sometimes inconsistencies. As an
example, reading data into R using base R will in general create
data.frames, but reading it using \texttt{dplyr} will create
\texttt{tibbles}. The way to manipulate these objects will differ too.

In general, things arguably look much better in the \texttt{tidyverse},
a poster child example being plots made in base R vs the
\texttt{tidyverse} \texttt{ggplot2} package.
...
have been exposed to the presentation at
\texttt{AQuikIntro2RandRStudioInQuarto.html}. If you are not in a class
room, you might want to take a look at it. This is also available at the
repository right about
\href{https://github.com/TiagoAMarques/AnIntro2RTutorial/blob/master/AQuikIntro2RandRStudioInQuarto.html}{here}.
This is an html file which you cannot visualize in github, so do
download it first to see it locally in your browser. Or, just see it at
\href{https://rpubs.com/talomarques/AnIntroToRandRStudio}{A Quick
...
An example screenshot of RStudio is below for reference. If you are not
seeing something like this, maybe you are not in RStudio ;)

\pandocbounded{\includegraphics[keepaspectratio]{images/clipboard-1423315731.png}}
...
\section{Dynamic reports and reproducible
...
how simple it becomes to work with dynamic reports, built on Quarto (if
you are an old geeser\ldots{} Quarto is like RMarkdown, on steroids -
with no side effects). This will take you to the next level in data
analysis. Actually, this document was itself created as a dynamic
report, using Quarto. You should explore some of the basics of R quarto,
and you can do so here:
https://quarto.org/docs/get-started/hello/rstudio.html.

Experiment yourself to create one. In RStudio, select \texttt{File} -
\textgreater{} \texttt{New\ file} -\textgreater{} ?, then just add a
title, something like ``My first dynamic report'' and see what happens.
Explore the content of the file just created and see what happens when
you press the RStudio button \texttt{knit}. Experiment with the created
document to try and change some of the output. Experiment in creating
output as an \texttt{.html}, as a \texttt{.pdf}, and even as a
\texttt{.docx} Word document.

Actually, a good way to learn and get up and running fast in Quarto is
by example. Hence, I have prepared a template that you can use to create
without effort a nice dynamic report. Feel free to explore the material
here:

\url{https://github.com/TiagoAMarques/template_4_quarto_report}

Just download all the files into a folder, \texttt{knit} the file
\texttt{QuartoTemplate.Rmd} and off you go.

Imagine the potential when you are analyzing real data, and the data
changes after your report is written!

\subsection{Exploring the use of dynamic
reports}\label{exploring-the-use-of-dynamic-reports}

This section contains a separate activity that illustrates by example
the usefulness of dynamic reports.

If you are working on this report in class, this might actually only be
done in a separate class, after you have progressed further in the
tutorial. Therefore, your teacher will let you know when it is the time
to work on this task. It is also possible that your teacher will have
shown you this or another similar example in class. If that is the case,
you could just use this material later when revising the concepts.

Check in folder \texttt{ExampleDataAnalysis} at
...
\tightlist
\item
  Download the file \texttt{ExampleDataAnalysis.Rmd}
\item
  Open \texttt{ExampleDataAnalysis.Rmd}
\item
  Compile (i.e.~knit) \texttt{ExampleDataAnalysis.Rmd}
\item
  Explore the (\texttt{.html}) output of the analysis
\item
  Implement the last few actions described in the final ``Your task''
  section of the document (which will lead to the creation of a new
  \texttt{.Rmd} file, \texttt{ExampleDataAnalysis2.Rmd})
\item
  Recompile the just created \texttt{ExampleDataAnalysis2.Rmd}
\item
...
\item
  an hypothetical example of when this might come in handy in your work.
\end{enumerate}

\section{A first quick session in
RStudio}\label{a-first-quick-session-in-rstudio}

Here we present a brief introduction to R inside RStudio, using a code
script and the command line.

In the coming sections we will mostly consider analysis using dynamic
reports via Quarto documents (.qmd), but if this is the first time you
use R, it is useful to start with a session where you can see objects
being created in the global environment via the command line.

Open RStudio. By default an empty workspace should appear. A workspace
is like your table in a library. You can work with all the objects in
your table. If you have an existing workspace, you can open it by
selecting \texttt{File\textbar{}Open\ File}. We recommend that you begin
by creating a new script file (RStudio Shortcut: \texttt{Ctrl+Shift+N};
this creates a .R file) and use that file to save, and comment, all the
code that you will create and execut during this current R session. In
this way you will have a record of everything you did, and you can save
that file as say \texttt{Session1.R} for later use.
...
complete, R presents the \texttt{+} character, so that you, the R user,
know R expects the conclusion of the current line.
...
something that R does for you and R will complain with an error message
if you try to do it yourself! Past experience tells me that more than
one person will have problems because they forgot to delete a
\texttt{\textgreater{}} and/or \texttt{+} from code when they copy paste
the code into their own R sessions. Avoid being that person!
...
should have no objects listed (there might be objects if you used R
before and there is already an \texttt{.Rdata} file in the working
folder). It will look like something as

\pandocbounded{\includegraphics[keepaspectratio]{extfiles/emptyenv.png}}

First we just create a couple of objects and use them, but below we will
...
If you wanted to work again with whatever you might have saved in your
workspace, you would do so by loading the saved \texttt{.Rdata}
workspace file

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{load}\NormalTok{(}\AttributeTok{file=}\StringTok{"my1stR.Rdata"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

as you can see, the objects you had created are back and available to
you
...
We will not be using these objects anymore, so feel free to delete them
as well as to delete the workspace file \texttt{my1stR.Rdata}.

Note that you saved your workspace in some directory, and loaded it up
again from there, but you have not defined the directory explicitly. By
default, this is your working directory. You can check what that
directory currently is by using the following command
...
\begin{verbatim}
[1] "C:/Users/tiago/Documents/GithubDesktop/AnIntro2RTutorial"
\end{verbatim}
...
extensions \texttt{.R} (a script), a \texttt{.Rdata} (a workspace) or a
\texttt{.qmd} (a dynamic report in Quarto).
...
Now you have used R in RStudio, lets use the power of their integration
to work directly in a dynamic report.

\section{Working through R via a dynamic report}\label{sec-starthere2}

Create a new dynamic report using a quarto file, as described above.
Call it something sensible, like \texttt{*****EN2026tutorial.qmd}.
Copy-paste into it as much as you want from this current file. A lazy
option would be to create a copy of this \texttt{.qmd} and rename it,
deleting most of the material above.

Comment all you do in the appropriate place: things that seem obvious to
you when you do them won't necessarily be obvious to you next week, so
describe with comments what they are and why you did them. Therefore, at
the end you will have a record that makes it easy to track everything
you did, and a template you can use in future classes.

Once you created the quarto file from scratch, we can start by creating
a new variable.
...
\texttt{Ctrl+Alt+I}. When code chucks are properly formatted they appear
highlighted in the .Rmd.

An empty code chunk (in the image with a comment added to it!) looks
like this:

\pandocbounded{\includegraphics[keepaspectratio]{extfiles/codechunk.JPG}}
...
sometimes referred to as a disadvantage by lets call them
``code-purists''. For simplicity, we deliberately avoid presenting the
several alternatives for each action, and concentrate on the ones we
prefer. This is not the same as saying these are the best, and if you
continue to work with R you will likely get used to doing things your
way - for now we do it our way!
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# do not have this line of code in a dynamic report}
\CommentTok{\# it will fire the help file into a browser}
\CommentTok{\# which is a bit of a pain}
\CommentTok{\# Note in my .Rmd this code has}
\CommentTok{\# eval=FALSE and so is not executed, just shown}
\NormalTok{?rm}
\end{Highlighting}
\end{Shaded}
...
one can search for functions containing a given string using the
\texttt{apropos} function. The following command lists all the functions
with the string \texttt{mean} in them.
...
Take a peak at the objects just created (important: you must type an
object name to see it printed on the output!):
...
numbers. The optional arguments \texttt{by}, \texttt{length.out} and
...
different arguments. In our experience \texttt{by} and
\texttt{length.out} (or \texttt{length} for short) will be more often
used than \texttt{along.with} (or \texttt{along} for short), but this
you will find out for yourself from experience.
...
if you just use the name of the object on the console it also gets
printed by default
...
is that R \emph{recycles} vectors if they are not the same length
...
\texttt{x9} were added. Usually \emph{error messages are important and
...
We have just created and removed some objects, and used simple functions
like \texttt{ls}, \texttt{seq} or \texttt{save}. R is an object oriented
language, and functions and vectors are just examples of types of
objects available in R. In the next section we go through the most
commonly used classes of objects in R.

\section{Types and classes of
...
\tightlist
\item
  \texttt{integer}, for integer numbers (could save memory with respect
  to the class numeric, if you only have integers)
\item
  \texttt{character}, representing for character strings, like ``words''
  say
\item
  \texttt{factor}, used to represent levels of a categorical variable,
  e.g.~car makes or models
\item
  \texttt{logical}, the logical values TRUE and FALSE
\item
  \texttt{matrix}, an object containing data in tabular form
\item
  \texttt{data.frame}, an object containing the kind of data you could
  have in a single Excel worksheet
\item
  \texttt{list}, an object that can be used to store all sorts of
  unstructured (or structured) data
\item
  \texttt{function}, all the functions in R, like those you just used
  \texttt{c}, \texttt{seq} or \texttt{rep} are of this class.
\end{itemize}

While many, maaaaany other classes of objects exist, these are arguably
some of those more commonly used.
...
the class of the objects that are used as their arguments. As an
example, note below how \texttt{summary} treats differently an object of
class \texttt{factor} or one of class \texttt{numeric}, producing a
table of counts per level for a factor but a 6 number summary for
numeric values.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{obj1}\OtherTok{\textless{}{-}}\FunctionTok{factor}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"a"}\NormalTok{,}\StringTok{"a"}\NormalTok{,}\StringTok{"b"}\NormalTok{,}\StringTok{"a"}\NormalTok{,}\StringTok{"b"}\NormalTok{,}\StringTok{"a"}\NormalTok{,}\StringTok{"a"}\NormalTok{,}\StringTok{"a"}\NormalTok{,}\StringTok{"a"}\NormalTok{,}\StringTok{"a"}\NormalTok{,}\StringTok{"e"}\NormalTok{,}\StringTok{"a"}\NormalTok{,}\StringTok{"f"}\NormalTok{,}\StringTok{"a"}\NormalTok{,}\StringTok{"f"}\NormalTok{,}\StringTok{"d"}\NormalTok{,}\StringTok{"a"}\NormalTok{,}\StringTok{"b"}\NormalTok{,}\StringTok{"d"}\NormalTok{,}\StringTok{"a"}\NormalTok{,}\StringTok{"b"}\NormalTok{,}\StringTok{"a"}\NormalTok{,}\StringTok{"a"}\NormalTok{,}\StringTok{"d"}\NormalTok{))}
...
second column (4,5,6) etc. If you want it to fill the matrix by rows,
i.e.~the first row, then the second, etc, you can use the optional
argument \texttt{byrow=TRUE}, like this:
...
cover those here, but look up the help on \texttt{array} if you are
...
via the installation of additional packages available online on CRAN
(The Comprehensive R Archive Network).

A package is just a set of functions and/or data sets (and the
corresponding documentation plus some additional required files) which
usually have some specific goal. As examples, in our course we will be
using general packages \texttt{lubridate}, great to manipulate dates and
times, \texttt{dplyr}, which extends data import and manipulation, as
well as \texttt{vegan} and \texttt{mgcv}, which allow the implementation
...
imagine. Therefore, before coding your own new method, check if it has
been coded by others before.

Installing a new package in R requires a call to function
\texttt{install.packages()}. A handy RStudio shortcut is simply to
follow the \texttt{Tools\textbar{}Install\ packages...} shortcut.
...
as an argument. In RStudio this can also be done by ticking the
corresponding package box under the RStudio tab ``Packages'' (by default
this tab is available on the bottom right window, between the ``Plots''
and ``Help'' tabs).

Here we use package \texttt{praise} as an example. You might be needing
it by now! Notice to begin with that \texttt{praise} is not available
yet (assuming you never installed it in the machine you are working on)
...
\begin{Highlighting}[]
\CommentTok{\# do not have this line of code in a dynamic report}
\CommentTok{\# it will install the package package}
\CommentTok{\# and that only needs to be done once}
\CommentTok{\# otherwise it would be like}
\CommentTok{\# installing word every time }
\CommentTok{\# you wanted to work on a word file}
\CommentTok{\# in the .Rmd the option is set as}
\CommentTok{\# eval=FALSE and so is not executed, just shown}
...
and finally we check that the \texttt{praise} functions are now loaded

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# praise() is actually the only function }
\CommentTok{\# meant to be used}
\CommentTok{\# by users in the praise package}
\CommentTok{\# do not have this line of code in a dynamic report}
\CommentTok{\# it will fire the help file into a browser}
\CommentTok{\# which is a bit of a pain}
\CommentTok{\# Note in my .Rmd this code has}
...
From now on, when you feel frustrated about R (or life in general\ldots)
you can just use R to uplift you

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# get uplifted}
...
Important note: you can and should load required packages in dynamic
reports via the function \texttt{library}. You should not install
packages (with \texttt{install.packages()}) within a dynamic report.
Install packages directly on the command line, and only once.

Otherwise, if doing it inside a dynamic report, that would be a akin to
installing Word every time you wanted to work on a Word document!
...
\def\labelenumi{\arabic{enumi}.}
\item
  Use the internet to find some useful package to do something you might
  want to do, the install the package and then use the example code in
  it.
\item
  Investigate the package \texttt{memer} to create memes from within R
  :)
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# if devtools is not installed, 1st install it}
\CommentTok{\# install.packages("devtools")}
\CommentTok{\# some packages are not available from CRAN}
\CommentTok{\# being available at their developers page}
\CommentTok{\# Many of these are hosted on github}
\CommentTok{\# Install the development version of memer from GitHub:}
\CommentTok{\# This might require you to install dependencies }
\CommentTok{\# (dependencis: other packages that a package needs)}
\CommentTok{\# and all that could take some time}
\CommentTok{\# so do not do this in class}
\NormalTok{devtools}\SpecialCharTok{::}\FunctionTok{install\_github}\NormalTok{(}\StringTok{"sctyner/memer"}\NormalTok{)}
\CommentTok{\# load the package}
\FunctionTok{library}\NormalTok{(memer)}
\CommentTok{\# Create a meme }
\FunctionTok{meme\_get}\NormalTok{(}\StringTok{"DistractedBf"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{meme\_text\_distbf}\NormalTok{(}\StringTok{"tidyverse"}\NormalTok{, }\StringTok{"new R users"}\NormalTok{, }\StringTok{"base R"}\NormalTok{)}
\CommentTok{\# how cool is that}
\CommentTok{\# challenge: send me a new meme that you create yourself}
\end{Highlighting}
\end{Shaded}

Both \texttt{memer} and \texttt{praise} are examples of ``fun''
packages. There are a few more you can explore
\href{https://medium.com/geekculture/15-fun-r-packages-you-may-not-know-of-fb25a9dcd627}{here}.

So, in fact, unlike what it might have been your first impression, R can
be fun :)
...
\textbf{Task 3}: Import the file \texttt{dados1.csv} into R, giving it
...
\pandocbounded{\includegraphics[keepaspectratio]{TAMsIntro2RviaRStudioTutorial_files/figure-pdf/unnamed-chunk-56-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{TAMsIntro2RviaRStudioTutorial_files/figure-pdf/unnamed-chunk-57-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{TAMsIntro2RviaRStudioTutorial_files/figure-pdf/unnamed-chunk-58-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{TAMsIntro2RviaRStudioTutorial_files/figure-pdf/unnamed-chunk-59-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{TAMsIntro2RviaRStudioTutorial_files/figure-pdf/unnamed-chunk-60-1.pdf}}

\textbf{Task 4}: Using data \texttt{cars}, create a plot that represents
...
\pandocbounded{\includegraphics[keepaspectratio]{TAMsIntro2RviaRStudioTutorial_files/figure-pdf/unnamed-chunk-61-1.pdf}}

Here and in class I will generally use R base for plots, but nowdays the
cool kids tend to use the ggplot2 system
(https://r-graph-gallery.com/ggplot2-package.html). It is worth to go
and find out about it, but also see next section for a sneack preview.
...
https://r-graph-gallery.com/ Lots of ideas, with the R code to use them
with your data!

\subsection{Tidyverse}\label{tidyverse}

Here I just present the above plots considering the \texttt{ggplot2}
syntax, so you can get used to seeing it both ways, and also starting to
decide which way you prefer to work yourself. First, we load up ggplot2
...
Then, we reproduce the five plots created above in base R. Do it
yourself, and changes as much as you can. Break it too, learn from
mistakes!
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(indat, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =} \FunctionTok{seq\_along}\NormalTok{(Sepal.Length), }\AttributeTok{y =}\NormalTok{ Sepal.Length)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Index"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Sepal.Length"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_bw}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{TAMsIntro2RviaRStudioTutorial_files/figure-pdf/unnamed-chunk-63-1.pdf}}
...
\NormalTok{ys}\OtherTok{\textless{}{-}}\NormalTok{indat}\SpecialCharTok{$}\NormalTok{Sepal.Length}
\NormalTok{xs}\OtherTok{\textless{}{-}}\NormalTok{indat}\SpecialCharTok{$}\NormalTok{Species}
\FunctionTok{ggplot}\NormalTok{(indat, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =} \FunctionTok{as.factor}\NormalTok{(xs), }\AttributeTok{y =}\NormalTok{ ys)) }\SpecialCharTok{+}
  \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"xs"}\NormalTok{, }\AttributeTok{y =} \StringTok{"ys"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_bw}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{TAMsIntro2RviaRStudioTutorial_files/figure-pdf/unnamed-chunk-64-1.pdf}}
...
\FunctionTok{ggplot}\NormalTok{(indat, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =} \FunctionTok{as.factor}\NormalTok{(xs), }\AttributeTok{y =}\NormalTok{ ys)) }\SpecialCharTok{+}
  \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"xs"}\NormalTok{,}
       \AttributeTok{y =} \StringTok{"Sepal Length (in mm)"}\NormalTok{,}
       \AttributeTok{title =} \StringTok{"Sepal length by species"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_bw}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{TAMsIntro2RviaRStudioTutorial_files/figure-pdf/unnamed-chunk-65-1.pdf}}
...
\NormalTok{p1 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(indat, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Sepal.Length)) }\SpecialCharTok{+}
  \FunctionTok{geom\_histogram}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Sepal.Length"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Frequency"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_bw}\NormalTok{()}

\NormalTok{p2 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(indat, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Sepal.Width)) }\SpecialCharTok{+}
  \FunctionTok{geom\_histogram}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Sepal.Width"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Frequency"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_bw}\NormalTok{()}

\NormalTok{p3 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(indat, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Petal.Length)) }\SpecialCharTok{+}
  \FunctionTok{geom\_histogram}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Petal.Length"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Frequency"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_bw}\NormalTok{()}

\NormalTok{p4 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(indat, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Petal.Width)) }\SpecialCharTok{+}
  \FunctionTok{geom\_histogram}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Petal.Width"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Frequency"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_bw}\NormalTok{()}

\NormalTok{p5 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(indat, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Petal.Length, }\AttributeTok{y =}\NormalTok{ Petal.Width)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{shape =} \DecValTok{21}\NormalTok{, }\AttributeTok{colour =} \StringTok{"cyan4"}\NormalTok{, }\AttributeTok{fill =} \StringTok{"green3"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Petal.Length"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Petal.Width"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_bw}\NormalTok{()}

\NormalTok{p6 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(indat, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Sepal.Length, }\AttributeTok{y =}\NormalTok{ Sepal.Width)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{shape =} \DecValTok{16}\NormalTok{, }\AttributeTok{colour =} \StringTok{"green3"}\NormalTok{) }\SpecialCharTok{+}
...
\pandocbounded{\includegraphics[keepaspectratio]{TAMsIntro2RviaRStudioTutorial_files/figure-pdf/unnamed-chunk-66-1.pdf}}
...
\FunctionTok{ggpairs}\NormalTok{(indat[, }\DecValTok{1}\SpecialCharTok{:}\DecValTok{4}\NormalTok{],}
        \AttributeTok{upper =} \FunctionTok{list}\NormalTok{(}\AttributeTok{continuous =} \StringTok{"points"}\NormalTok{),}
        \AttributeTok{diag  =} \FunctionTok{list}\NormalTok{(}\AttributeTok{continuous =} \StringTok{"blankDiag"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{TAMsIntro2RviaRStudioTutorial_files/figure-pdf/unnamed-chunk-67-1.pdf}}
...
\NormalTok{pesos }\OtherTok{\textless{}{-}} \FunctionTok{rnorm}\NormalTok{(}\DecValTok{100}\NormalTok{, }\DecValTok{200}\NormalTok{, }\DecValTok{20}\NormalTok{)}
\NormalTok{sps   }\OtherTok{\textless{}{-}} \FunctionTok{rep}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"Carapau"}\NormalTok{, }\StringTok{"Sardinha"}\NormalTok{), }\AttributeTok{each =} \DecValTok{50}\NormalTok{)}
\NormalTok{df    }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(pesos, sps)}

\CommentTok{\# Boxplot 1 {-} horizontal x{-}axis labels (las=1)}
\NormalTok{p1 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(df, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ sps, }\AttributeTok{y =}\NormalTok{ pesos)) }\SpecialCharTok{+}
  \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Espécie"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Peso"}\NormalTok{) }\SpecialCharTok{+}
  ...
  \FunctionTok{theme}\NormalTok{(}\AttributeTok{axis.text.x =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{angle =} \DecValTok{0}\NormalTok{, }\AttributeTok{hjust =} \FloatTok{0.5}\NormalTok{),}
        \AttributeTok{plot.margin =} \FunctionTok{margin}\NormalTok{(}\AttributeTok{b =} \DecValTok{40}\NormalTok{))}

\CommentTok{\# Boxplot 2 {-} vertical x{-}axis labels (las=2) + green fill}
\NormalTok{p2 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(df, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ sps, }\AttributeTok{y =}\NormalTok{ pesos)) }\SpecialCharTok{+}
  \FunctionTok{geom\_boxplot}\NormalTok{(}\AttributeTok{fill =} \StringTok{"green3"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Espécie"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Peso"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_bw}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{theme}\NormalTok{(}\AttributeTok{axis.text.x =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{angle =} \DecValTok{90}\NormalTok{, }\AttributeTok{hjust =} \DecValTok{1}\NormalTok{),}
        \AttributeTok{plot.margin =} \FunctionTok{margin}\NormalTok{(}\AttributeTok{b =} \DecValTok{40}\NormalTok{))}

\CommentTok{\# Scatter plot with 4 point layers}
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{1234}\NormalTok{)}
\NormalTok{df2 }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}
  \AttributeTok{x   =} \FunctionTok{c}\NormalTok{(}\FunctionTok{rnorm}\NormalTok{(}\DecValTok{10}\NormalTok{), }\FunctionTok{rnorm}\NormalTok{(}\DecValTok{10}\NormalTok{), }\FunctionTok{rnorm}\NormalTok{(}\DecValTok{10}\NormalTok{), }\FunctionTok{rnorm}\NormalTok{(}\DecValTok{10}\NormalTok{)),}
  \AttributeTok{y   =} \FunctionTok{c}\NormalTok{(}\FunctionTok{rnorm}\NormalTok{(}\DecValTok{10}\NormalTok{), }\FunctionTok{rnorm}\NormalTok{(}\DecValTok{10}\NormalTok{), }\FunctionTok{rnorm}\NormalTok{(}\DecValTok{10}\NormalTok{), }\FunctionTok{rnorm}\NormalTok{(}\DecValTok{10}\NormalTok{)),}
  \AttributeTok{grp =} \FunctionTok{rep}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"A"}\NormalTok{,}\StringTok{"B"}\NormalTok{,}\StringTok{"C"}\NormalTok{,}\StringTok{"D"}\NormalTok{), }\AttributeTok{each =} \DecValTok{10}\NormalTok{)}
\NormalTok{)}

\NormalTok{p3 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(df2, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ x, }\AttributeTok{y =}\NormalTok{ y)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{data =} \FunctionTok{subset}\NormalTok{(df2, grp }\SpecialCharTok{==} \StringTok{"A"}\NormalTok{),}
             \AttributeTok{colour =} \StringTok{"blue"}\NormalTok{,   }\AttributeTok{size =} \DecValTok{4}\NormalTok{,  }\AttributeTok{shape =} \DecValTok{16}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{data =} \FunctionTok{subset}\NormalTok{(df2, grp }\SpecialCharTok{==} \StringTok{"B"}\NormalTok{),}
             \AttributeTok{colour =} \StringTok{"magenta"}\NormalTok{, }\AttributeTok{size =} \DecValTok{4}\NormalTok{, }\AttributeTok{shape =} \DecValTok{2}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{data =} \FunctionTok{subset}\NormalTok{(df2, grp }\SpecialCharTok{==} \StringTok{"C"}\NormalTok{),}
             \AttributeTok{colour =} \StringTok{"orange"}\NormalTok{, }\AttributeTok{size =} \DecValTok{8}\NormalTok{,  }\AttributeTok{shape =} \DecValTok{17}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{data =} \FunctionTok{subset}\NormalTok{(df2, grp }\SpecialCharTok{==} \StringTok{"D"}\NormalTok{),}
             \AttributeTok{colour =} \StringTok{"blue"}\NormalTok{,   }\AttributeTok{size =} \DecValTok{6}\NormalTok{,  }\AttributeTok{shape =} \DecValTok{21}\NormalTok{, }\AttributeTok{fill =} \StringTok{"green"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{xlim}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{3}\NormalTok{, }\DecValTok{3}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylim}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{3}\NormalTok{, }\DecValTok{3}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_bw}\NormalTok{()}
...
\pandocbounded{\includegraphics[keepaspectratio]{TAMsIntro2RviaRStudioTutorial_files/figure-pdf/unnamed-chunk-68-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{TAMsIntro2RviaRStudioTutorial_files/figure-pdf/chunck159-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{TAMsIntro2RviaRStudioTutorial_files/figure-pdf/unnamed-chunk-72-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{TAMsIntro2RviaRStudioTutorial_files/figure-pdf/unnamed-chunk-74-1.pdf}}

\section{Dates and Times}\label{dates-and-times}

We might sometimes need to work with dates and/or times, and this used
to be a major pain back in the day. Thanks to the functionality of
package \texttt{lubridate}, that is no longer the case.
...
Dates and times are natively of class POSIXct, POSIXt in R. As an
example, take a look at the current date and time using function
\texttt{Sys.Date()}. It is now (as in, when this document was last
compiled!)

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#for just the date try }
\CommentTok{\# Sys.Date()}
\NormalTok{dtnow }\OtherTok{\textless{}{-}} \FunctionTok{Sys.time}\NormalTok{()}
\NormalTok{dtnow}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
[1] "2026-09-20 13:43:13 WEST"
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{class}\NormalTok{(dtnow)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{as.Date}\NormalTok{(dtnow)  }\CommentTok{\# R grabs the date part automatically}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{day}\NormalTok{(dtnow)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{yday}\NormalTok{(dtnow)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dtnow }\OtherTok{\textless{}{-}} \FunctionTok{ymd\_hms}\NormalTok{(dtnow, }\AttributeTok{tz =} \StringTok{"Europe/Lisbon"}\NormalTok{)}
\FunctionTok{Sys.time}\NormalTok{()}\SpecialCharTok{{-}}\NormalTok{dtnow}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
Time difference of 0.04928803 secs
\end{verbatim}

You can also turn strings (aka characters) into date-time objects, as an
example, this is my birthday as text

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{bday }\OtherTok{\textless{}{-}} \FunctionTok{mdy}\NormalTok{(}\StringTok{"July 22nd, 1975"}\NormalTok{)}
\NormalTok{bday}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{Sys.Date}\NormalTok{()}\SpecialCharTok{{-}}\NormalTok{bday}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
Time difference of 18688 days
\end{verbatim}

a bit nicer and controling units, my age in years is

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{as.numeric}\NormalTok{(}\FunctionTok{interval}\NormalTok{(bday, }\FunctionTok{Sys.Date}\NormalTok{()), }\StringTok{"years"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Integer age (completed years)}
\FunctionTok{interval}\NormalTok{(bday, }\FunctionTok{Sys.Date}\NormalTok{()) }\SpecialCharTok{\%/\%} \FunctionTok{years}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Check out
\href{https://rstudio.github.io/cheatsheets/html/lubridate.html}{here}
the many other amazing things you can do with times and dates leveraging
on \texttt{lubridate} functionality.
...
\pandocbounded{\includegraphics[keepaspectratio]{TAMsIntro2RviaRStudioTutorial_files/figure-pdf/unnamed-chunk-96-1.pdf}}
...
course of data analysis in R could take a lifetime.

Our objective with this tutorial was simply to introduce you to R such
that when we use R in the next few days/weeks, for whatever course you
might be taking, the commands do not look too esoteric. Nonetheless,
this material, as well as the references provided, should constitute a
good basis to learn R further if you so desire.
...
Ecologia Numérica courses at DBA, FCUL, or doing internships / being
supervised by me. I thank their kind contributions here: Danielle
Harris, Len Thomas, Soraia Pereira, Susana França, Sofia Reboleira,
Sónia Coelho, Afonso Barrocal, José Pedro Granadeiro, Daniela Leal, Nuno
Escamade and Theoni Photopoulou. Many are not named explicitly, as I've
forgotten about the specifics, but I thank them any way! If you think
your name is missing, let me know, and I will add it here. Further, this
list is ever evolving and so, if you have comments, please, send them my
way and get your name added to it! Indeed, fame for eternity is indeed
that close :)
