...
You might be reading this document as a pdf, a word or an html. These files are created by compiling (i.e. knitting) a quarto dynamic report (see many more details about dynamic reports below). The .qmd "TAMsIntro2RviaRStudioTutorial.qmd", which after compilation, creates these 3 different formats, is available in the github repository located at:

<https://github.com/TiagoAMarques/AnIntro2RTutorial>

## Tutorial versions

This .qmd was last compiled on `r Sys.time()`. The latest version of this tutorial is always available at the [github repository](https://github.com/TiagoAMarques/AnIntro2RTutorial).
...
This tutorial assumes that R and RStudio have been previously installed in the computer you are using. The latest version of both software packages is recommended. Both are free and open source. You can get R [here](https://www.r-project.org/) and Rstudio [here](https://www.rstudio.com/).
...
To facilitate the interaction with R we leverage on [RStudio](https://www.rstudio.com/), a graphical user interface (GUI), a piece of software which allows users to have at a click's distance many useful features of R. In the following sections of the tutorial you will be guided through a first session of R via RStudio, and then you will start working on a dynamic report.

The tutorial is intended to follow a presentation about R and RStudio, their interaction and capabilities ("AQuikIntro2RandRStudioInQuarto.html"). This document is also in the git repository, but the presentation itself is also hosted at RPubs: [A Quick Introduction to R and RStudio](https://rpubs.com/talomarques/1342657) where you can view it immediately.

If you are using this tutorial in class with me/a teacher, after you are exposed to the slides, you might want to jump straight to [Working through R via a dynamic report](#sec-starthere2). Do come back after class to all the material in between here and there, as it might contain useful context and additional relevant material. Remember, here you learn as much as you want to, and those that want the most will certainly do better.
...
This document is written in Quarto, a tool that allows you to build dynamic reports based on R code, providing integrated documents that contain all that is required for a given project, from reading the data in to final results and discussion, passing through all the analysis and results. If you want a gentle introduction to Quarto using a hands on tutorial based on a versatile template that will do many of the things you will need to get started, look for no more, there is one here:

<https://github.com/TiagoAMarques/template_4_quarto_report>
...
- [Cheat sheets](https://rstudio.github.io/cheatsheets/) - an incredible useful set of resources from the RStudio team, where self contained subject specific sets of functions are provided for different common tasks. Because of their importance for our work, we highlight a few here:

  - [Base R](https://iqss.github.io/dss-workshops/R/Rintro/base-r-cheat-sheet.pdf): the basics in base R
  - [quarto](https://rstudio.github.io/cheatsheets/quarto.pdf): making dynamic reports in quarto
  - [lubridate](https://rstudio.github.io/cheatsheets/html/lubridate.html): manipulating and formating dates and times
  - [ggplot](https://rstudio.github.io/cheatsheets/html/data-visualization.html): plotting data
  - [dplyr](https://rstudio.github.io/cheatsheets/html/data-transformation.html): manipulating data
  - [sf](https://rstudio.github.io/cheatsheets/sf.pdf): manipulating spatial objects

Additionally, a large number of [contributed cheat sheets](https://rstudio.github.io/cheatsheets/contributed-cheatsheets.html) also exist.

At the [landing page](https://github.com/TiagoAMarques/AnIntro2RTutorial) of the github repository hosting this tutorial there is a longer list of less introductory/general resources on R that might just have what you were looking for. Disclaimer: said list corresponds to a random non-exhaustive list of resources I have read and were useful to me at some point. I make no claims they might be useful to you :)
...
The `tidyverse` way can be implemented via a suite of R packages, and you can read all about it in this [book](https://r4ds.hadley.nz/).

![](images/clipboard-2143144059.png)
...
Typically, if I am using this tutorial in a class room, the student will have been exposed to the presentation at `AQuikIntro2RandRStudioInQuarto.html`. If you are not in a class room, you might want to take a look at it. This is also available at the repository right about [here](https://github.com/TiagoAMarques/AnIntro2RTutorial/blob/master/AQuikIntro2RandRStudioInQuarto.html). This is an html file which you cannot visualize in github, so do download it first to see it locally in your browser. Or, just see it at [A Quick Introduction to R and RStudio](https://rpubs.com/talomarques/AnIntroToRandRStudio).
...
![](images/clipboard-1423315731.png)
...
One of the most amazing features of the integration of R and RStudio is how simple it becomes to work with dynamic reports, built on Quarto (if you are an old geeser... Quarto is like RMarkdown, on steroids - with no side effects). This will take you to the next level in data analysis. Actually, this document was itself created as a dynamic report, using Quarto. You should explore some of the basics of R quarto, and you can do so here: https://quarto.org/docs/get-started/hello/rstudio.html.

Experiment yourself to create one. In RStudio, select `File` - \> `New file` -\> ?, then just add a title, something like "My first dynamic report" and see what happens. Explore the content of the file just created and see what happens when you press the RStudio button `knit`. Experiment with the created document to try and change some of the output. Experiment in creating output as an `.html`, as a `.pdf`, and even as a `.docx` Word document.

Actually, a good way to learn and get up and running fast in Quarto is by example. Hence, I have prepared a template that you can use to create without effort a nice dynamic report. Feel free to explore the material here:

<https://github.com/TiagoAMarques/template_4_quarto_report>

Just download all the files into a folder, `knit` the file `QuartoTemplate.Rmd` and off you go.
...
If you are working on this report in class, this might actually only be done in a separate class, after you have progressed further in the tutorial. Therefore, your teacher will let you know when it is the time to work on this task. It is also possible that your teacher will have shown you this or another similar example in class. If that is the case, you could just use this material later when revising the concepts.
...
In the coming sections we will mostly consider analysis using dynamic reports via Quarto documents (.qmd), but if this is the first time you use R, it is useful to start with a session where you can see objects being created in the global environment via the command line.
...
A good trick to make sure you are working in the directory you want is to open RStudio from the directory you want it to be working on. You can do that by double clicking a file R studio recognizes as a file it should open, e.g. as might happen in particular for files with extensions `.R` (a script), a `.Rdata` (a workspace) or a `.qmd` (a dynamic report in Quarto).
...
# Working through R via a dynamic report {#sec-starthere2}

Create a new dynamic report using a quarto file, as described above. Call it something sensible, like `*****EN2026tutorial.qmd`. Copy-paste into it as much as you want from this current file. A lazy option would be to create a copy of this `.qmd` and rename it, deleting most of the material above.

Comment all you do in the appropriate place: things that seem obvious to you when you do them won't necessarily be obvious to you next week, so describe with comments what they are and why you did them. Therefore, at the end you will have a record that makes it easy to track everything you did, and a template you can use in future classes.

Once you created the quarto file from scratch, we can start by creating a new variable.

Note that all the code must go inside code chunks, and you can get them by doing `Code | InsertChunk` or the shortcut `Ctrl+Alt+I`. When code chucks are properly formatted they appear highlighted in the .Rmd.

An empty code chunk (in the image with a comment added to it!) looks like this:

![](extfiles/codechunk.JPG)
...
```{r}
obj1<-factor(c("a","a","b","a","b","a","a","a","a","a","e","a","f","a","f","d","a","b","d","a","b","a","a","d"))
summary(obj1)
...
Otherwise, if doing it inside a dynamic report, that would be a akin to installing Word every time you wanted to work on a Word document!

**Task 2**:

1.  Use the internet to find some useful package to do something you might want to do, the install the package and then use the example code in it.
...
# (dependencis: other packages that a package needs)
...
Both `memer` and `praise` are examples of "fun" packages. There are a few more you can explore [here](https://medium.com/geekculture/15-fun-r-packages-you-may-not-know-of-fb25a9dcd627).
...
Here and in class I will generally use R base for plots, but nowdays the cool kids tend to use the ggplot2 system (https://r-graph-gallery.com/ggplot2-package.html). It is worth to go and find out about it, but also see next section for a sneack preview.
...
We might sometimes need to work with dates and/or times, and this used to be a major pain back in the day. Thanks to the functionality of package `lubridate`, that is no longer the case.
...
Dates and times are natively of class `r class(Sys.time())` in R. As an example, take a look at the current date and time using function `Sys.Date()`. It is now (as in, when this document was last compiled!)
...
Check out [here](https://rstudio.github.io/cheatsheets/html/lubridate.html) the many other amazing things you can do with times and dates leveraging on `lubridate` functionality.
