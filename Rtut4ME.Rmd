...
urlcolor: blue
csl: mee.csl
subtitle: Modelaço Ecológica 2019/2020
...
This tutorial was created as a gentle introduction to the R environment via the R Studio interface to R. While it could be a general introduction to R, the primary objective of this document is to serve as a "hands-on-tutorial" for the course of "Modela??o Ecol?gica". It does not assume any  knowledge about R, but some basic programming notions would be desirable.
...
Once you created the RMardown from scratch, your workspace is tipically empty, but we can change that easily by creating new objects. We will create a variable called `myvar1` which we will assign the value of 4. This is typically done using the assign operator `<-`.
...
The function `seq` is very useful for setting sequences of numbers. The optional arguments `length.out` and `along.with` provide extra flexibility. 
